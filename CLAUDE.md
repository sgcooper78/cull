...
- **Packaging — one runnable file per platform, nothing else:** `Cull-<v>-portable.exe` (Windows, Enigma Virtual Box), `Cull-<v>-macos-{arm64,x64}.dmg`, `Cull-<v>-linux-x64.AppImage`. Scripts in `tool\` → `dist\` (gitignored): `build_portable.ps1` (requires `choco install enigmavirtualbox`; `gen_evb.ps1` builds the `.evb` from real output, no GUI step; fatal if Enigma missing/rejects), `package_macos.sh [arm64|x64]`, `package_linux.sh`. CI: `.github/workflows/release.yml` — matrix (windows / macos-14 / macos-13 / ubuntu) on `v*` tags + `workflow_dispatch`. Tag → `release` job publishes the files; manual run → downloadable workflow artifacts only. Runners have every toolchain — no local Mac/Linux needed. **macOS/Linux builds are UNVERIFIED** (never run on the dev box); first CI run is the real test — may need a macOS deployment-target bump or a Linux `-dev` package. Full guide `docs/RELEASING.md`.
