...
description: Handles OS-facing work — filesystem access and deletion semantics, the FileSource abstraction, method channels, plugin selection/setup, Windows runner config, and packaging (msix). Use for lib/data/fs/, lib/services/, and platform folders like windows/.
...
- Packaging: `flutter build windows`; set up `msix` config when requested. Keep `windows/runner` changes minimal and explain each one.
